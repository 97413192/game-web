...
public class DoubleServiceImpl implements DoubleService {
	...
	public String selectAllDouble(HttpServletRequest req) throws ServletException, IOException {
		...
		if(admin == null){
			...
			log.setDsc(Code.SELECT_ALLDOUBLE_OUTTIME);
		...
		try{
			...
			if (null == loginServerRMI) {
				//将获取登陆服务失败
				log.setStatus(ManagerLog.error);
				log.setDsc(Code.SELECT_ALLDOUBLE_LOGSERVER_ERROR);
				logMapper.logSave(log);
				
				return "waitDeskManagement/selectAllWaitDesk";
			}else{
				//传入一个状态码，返回一个RMIResult实体类，RMIResult的result属性为List<Double>
				...
				if(result.getErrorCode() == D.CODE_SUC && result.getResult() != null ){ //判断查询是否成功
					req.setAttribute("doubleList", (List<DoubleChance>)result.getResult());
					
					//将所有比倍查询成功存入到日志表中
					log.setStatus(ManagerLog.success);
					log.setDsc(Code.SELECT_ALLDOUBLE_SUC_CREROOM);
					logMapper.logSave(log);
					return "doubleManagement/selectAllDouble";
				}else{
					//将玩家查询失败存入到日志表中
					log.setStatus(ManagerLog.error);
			...
			//将由于未知错误引起的操作失败存入日志
			log.setStatus(ManagerLog.error);
			log.setDsc(Code.SELECT_ALLDOUBLE_UNKNOWN_ERROR);
			logMapper.logSave(log);
		}
		return "doubleManagement/selectAllDouble";
	}
	...
	public Result alterDouble(HttpServletRequest req, int chance,int id) throws ServletException, IOException {
		...
		if(admin == null){
			...
			log.setTime(time);
			log.setStatus(ManagerLog.error);
			log.setDsc(Code.ALTER_DOUBLE_OUTTIME);
			logMapper.logSave(log);
		...
		try{
			...
			if (null == loginServerRMI) {
				//获取登陆服务失败
				log.setStatus(ManagerLog.error);
				log.setDsc(Code.ALTER_DOUBLE_LOGSERVER_ERROR);
				logMapper.logSave(log);
				
				//返回结果
				result.setMsg(Code.ALTER_DOUBLE_LOGSERVER_ERROR);
				result.setStatus(1);
				return result;
			}else{
				//传入一个修改获胜概率的状态码及比倍id，chance，通过id的值修改chance。返回成功失败两种状态。
				RMIResult result1 = (RMIResult) loginServerRMI.exce(LoginRMIConstant.GM_TOOL_USR_INSERT_BY_NAME_PWD,id,chance);
				if(result1.getErrorCode() == D.CODE_SUC){
					//将修改成功存入日志
					log.setStatus(ManagerLog.success);
					log.setDsc(Code.ALTER_DOUBLE_SUC_CREROOM);
					logMapper.logSave(log);
					
					//返回结果
					result.setMsg(Code.ALTER_DOUBLE_SUC_CREROOM);
					result.setStatus(0);
					return result;
				}else{
					//打印失败状态码
					...
					//将修改失败加入的日志中
					log.setStatus(ManagerLog.error);
					log.setDsc(Code.ALTER_DOUBLE_GETRESULT_ERROR);
					logMapper.logSave(log);
					
					//返回结果
					result.setMsg(Code.ALTER_DOUBLE_GETRESULT_ERROR);
					result.setStatus(2);
					return result;
				}
			}
			...
			//将由于未知错误引起的操作失败存入日志
			log.setStatus(ManagerLog.error);
			log.setDsc(Code.ALTER_DOUBLE_UNKNOWN_ERROR);
			logMapper.logSave(log);
			
			//返回结果
			result.setMsg(Code.ALTER_DOUBLE_UNKNOWN_ERROR);
			result.setStatus(3);
		}
		return result;
	}
