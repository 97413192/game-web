		...
		$("#anpPageIndex_btn").click(function(){
			...
			if(!isNaN(jump)){
				if(jump>0 && jump<=${pagecount }){
					window.location.href = "agentDeal/findAll.do?index="+jump;
				}
			}
		});
		//给新增添加单击事件
		$('#toAddAgent').click(function(){
			window.location.href = "agentDeal/toAddAgent.do";
		});
	});
...
<body>
    ...
    <table class="imagetable" id="table1" width="100%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolor="A4B6D7"
        bgcolor="F2F8FF" class="admin_table">
        <tr>
            <td class="title_03" colspan="19" align="center">
                <strong><font color="red" size="4px">代理商交易记录</font></strong></td>
        </tr>
        ...
        <c:forEach items="${list }" var="log" varStatus="s">
        	<tr style="cursor:hand " onmousemove="changeTrColor(this)">
	             ...
	             <td ><input type="button" value="查看" class="btn btn-info" onclick="lookMessage(this)"></td>
        	</tr>
        </c:forEach>
        ...
        <tr>
			...
			<div style="width:60%;float:left;">
				...
				<c:when test="${index==1 }">
						...
						<a href="agentDeal/findAll.do?index=${index-1 }" style="margin-right:5px;">上一页</a>
								</c:otherwise> 
				</c:choose> 
							...
							<c:choose>
								<c:when test="${index == s.index }">
									<span style="margin-right:5px;font-weight:Bold;color:red;">${s.index }</span>
 										...
 										<c:otherwise>
 											<a href="agentDeal/findAll.do?index=${s.index }" style="margin-right:5px;">${s.index }</a>
 										</c:otherwise> 
							</c:choose>
						</c:forEach>
					</c:otherwise>
				...
				<c:choose>
					<c:when test="${pagecount == index }">
						...
						<span style="margin-right:5px;font-weight:Bold;color:red;">尾页</span>
								...
								<c:otherwise>
									<a href="agentDeal/findAll.do?index=${index+1 }" style="margin-right:5px;">下一页</a>
									<a href="agentDeal/findAll.do?index=${pagecount }" style="margin-right:5px;">尾页</a>
								</c:otherwise> 
				</c:choose>
    ...
    <div id="seeReason" style="display: none;">
    	<input type="button" value="返 回" onclick="returnTable()"/>
    	<textarea name="AdminName" cols="50" rows="5" id="text" class="btn btn-info" ></textarea>
