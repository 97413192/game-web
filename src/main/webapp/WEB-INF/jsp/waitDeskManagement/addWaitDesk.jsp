...
<script type="text/javascript">
	...
	function check(){
		...
		if(count>0){
			document.all.CreateWaitDesk.disabled = "disabled";
		}else{
			document.all.CreateWaitDesk.disabled = "";
		}
	}
	$(function(){
		//给创建按钮绑定单击事件
		$("#CreateWaitDesk").click(function(){
			alert("asdfasd");
			//获取输入的参数
			...
			//发送ajax请求
			$.ajax({
				url:"waitDesk/addWaitDesk.do",
				type:"post",
				data:{"time":time,"gold":gold,"typeName":typeName,"typeId":typeId},
				dataType:"json",
				success:function(result){
					if(result.status == 0){
						...
						alert(result.msg);
					}else if(result.status == 2){//通过登陆服务增加桌子错误
						alert(result.msg);
		...
		<table width="100%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolor="A4B6D7"
			...
			<tr>
				...
				<td colspan="2" align="left"  style="height:20px;">
					...
					<span id="timemsg" class="red">留桌的时间，时间使用单位秒</span>
				</td>
			</tr>
			<tr>
				<td align="right">
				...
				<td colspan="2" align="left">
					<input name="gold" type="text" id="gold" class="put" maxlength="12" 
						onkeyup="filterInput();check();"/>
					<span id="goldmsg" class="red">留桌消耗的金币</span>
				</td>
			</tr>
			<tr>
				<td align="right">
				...
				<td colspan="2" align="left">
					<input name="typeName" type="text" id="typeName" class="put" maxlength="12" 
						onkeyup="check();"/>
					<span id="typeNamemsg" class="red">留桌的类型名称，用于调用确认对应房间使用的是否是对应的留桌配置</span>
				</td>
			</tr>
			<tr>
				<td align="right">
				...
				<td colspan="2" align="left">
					<input name="typeId" type="text" id="typeId" class="put" maxlength="12" 
						onkeyup="filterInput();check();"/>
					<span id="typeIdmsg" class="red">留桌的类型编号，用于调用确认对应房间使用的是否是对应的留桌配置</span>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="2" align="left">
					<input type="button" name="CreateWaitDesk" value=" 创 建 " id="CreateWaitDesk" disabled = "disabled"/>
