...
<body>
    <form name="form1" method="post" action=" id="form1">
        <table class="imagetable" width="100%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolor="A4B6D7"
	            ...
	            <c:forEach items="${list}" var="realName" varStatus="s">
	            	<tr>
		                ...
		                <td>
		                 ...
		                 <td>
		                 	<c:if test="${realName.realNameInfo.applyTime == realName.realNameInfo.approveTime}">
		                 		<fmt:formatDate value="${realName.realNameInfo.approveTime}" pattern="yyyy-MM-dd HH:mm:ss" type="time" />(未批准)
		                 	</c:if>
		                 	<c:if test="${realName.realNameInfo.applyTime != realName.realNameInfo.approveTime}">
		                 		<fmt:formatDate value="${realName.realNameInfo.approveTime}" pattern="yyyy-MM-dd HH:mm:ss" type="time" />
		                 	</c:if>
		                 </td>
		                <td>
		                	<a class="btn btn-info agree" href="javascript:void(0)" id="${realName.gameId}">同意</a>
				 
				 			<a class="btn btn-danger refuse" href="javascript:void(0)" id="${realName.gameId}">不同意</a>
		                </td>
		                </tr>
	            </c:forEach>
            </c:if>
            <c:if test="${realNames.size() == 0}">
	            		<tr>
		            		<td colspan="9" style="text-align:center">暂无多余信息</td>
		                </tr>
            </c:if>
		                ...
		                <div id="anpPageIndex">
							...
							<div style="width:60%;float:left;">
								<c:choose>
    								...
    								<c:otherwise>
    									<a href="gamePlayer/realNameApply.do?index=1?index=1" style="margin-right:5px;">首页</a>
										<a href="gamePlayer/realNameApply.do?index=1?index=${index-1 }" style="margin-right:5px;">上一页</a>
    								</c:otherwise> 
								</c:choose> 
									...
									<c:when test="${pagecount<=10 }">
										<c:forEach begin="1" end="${pagecount }" varStatus="s">
											<c:choose>
	    										...
	    										<c:otherwise>
	    											<a href="gamePlayer/realNameApply.do?index=1?index=${s.index }" style="margin-right:5px;">${s.index }</a>
	    										</c:otherwise> 
											</c:choose>
										</c:forEach>
									</c:when>
								...
								<c:choose>
    								...
    								<c:otherwise>
    									<a href="gamePlayer/realNameApply.do?index=1?index=${index+1 }" style="margin-right:5px;">下一页</a>
    									<a href="gamePlayer/realNameApply.do?index=1?index=${pagecount }" style="margin-right:5px;">尾页</a>
    								</c:otherwise> 
								</c:choose>
	...
	$(function(){
		//href="information/toSelectInformation.do?
		 UPDATE_APPLYINFO_SUCCESS = "更新信息成功";
		 UPDATE_APPLYINFO_ERROR = "更新信息失败";
		//给确定添加加单击事件
		$(".agree").click(function(){
			var gameId = $(".agree").attr("id")
			var state  = 2
			if(confirm("是否同意该玩家的实名认证申请?")){
					//同意，更新该玩家的认证申请，更新批准时间，向前端发送钻石奖励
					$.ajax({
						type:'post',
